"""加载 checkpoint，在任意 split 上跑评估（frontend_exports 版）.
...
def main():
    ...
    hidden = args.hidden if args.hidden is not None else ck_args.get("hidden", 384)
    conv = ck_args.get("conv_blocks", 3)
    hl = ck_args.get("hist_layers", 4)
    hh = ck_args.get("hist_heads", 8)
    print(f"[ckpt] {args.ckpt}  epoch={ckpt.get('epoch')}  hidden={hidden}")

    device = torch.device(args.device)
    model = PointerPolicy(hidden=hidden, conv_blocks=conv, hist_layers=hl, hist_heads=hh).to(device)
