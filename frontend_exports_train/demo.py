...
示例：
    ...
    python demo.py --task response_choice           # 只看响应选择任务
...
from data import DEFAULT_DATA_DIR, PHASES, JsonlDataset, collate_fn, featurize, split_path
...
def fmt_action(a: dict) -> str:
    ...
    if t == "CHI":
        ...
        chi = a.get("chi_cards", []) or a.get("use_cards", []) or []
        bi = a.get("bi_cards", []) or []
        s = f"CHI 吃 {card_name(tgt).strip()} 用 {fmt_cards(chi)}"
        if bi:
            s += f"（比 {fmt_cards(bi)}）"
        return s
    if t == "GUO":
        miss = "/".join(a.get("missed_actions", []) or [])
        return f"GUO 过 (放弃 {miss} {card_name(a.get('target_card')).strip()})"
    if t in ("HU", "PENG", "ZHAO", "LONG", "BAO"):
        return f"{t} {card_name(a.get('target_card') or a.get('card')).strip()}"
    return f"{t} {a}"
...
def print_sample(sample: dict, probs: torch.Tensor, pred_idx: int) -> None:
    ...
    label = sample.get("label_index", sample.get("human_selected_index", 0))
    phase = obs.get("phase", "?")
    ...
    print(f"game_id   : {sample.get('game_id')}   replay={sample.get('replay_id')}   "
          f"task={sample.get('feedback_task')}")
    ...
    if tg:
        ...
        for tg_pid, groups in tg.items():
            if not groups:
                continue
            tag = "[自家]" if str(tg_pid) == str(pid) else "      "
            for g in groups:
                print(f"  {tag} pid={str(tg_pid):>10s}  {g.get('type','?'):6s} {fmt_cards(g.get('cards', []))}")

    for d in obs.get("discards") or []:
        cs = d.get("cards") or []
        if cs:
            tag = "[自家]" if d.get("player_id") == pid else "      "
            print(f"弃牌      :  {tag} pid={d.get('player_id'):>10}  {fmt_cards(cs)}")

    ld = obs.get("last_discard") or {}
    if ld:
        print(f"上一弃牌  : pid={ld.get('player_id')} -> {card_name(ld.get('card')).strip()}")
    if obs.get("pending_card"):
...
def pick_indices(ds: JsonlDataset, path: Path, n: int, phase: str | None,
                 task: str | None, seed: int) -> list[int]:
    rng = random.Random(seed)
    if phase is None and task is None:
        return [rng.randrange(0, len(ds)) for _ in range(n)]
    picked: list[int] = []
    tried = 0
    while len(picked) < n and tried < n * 400:
        tried += 1
        idx = rng.randrange(0, len(ds))
        raw = load_raw(path, ds.offsets[idx])
        if phase is not None and raw.get("observation", {}).get("phase") != phase:
            continue
        if task is not None and raw.get("feedback_task") != task:
            continue
        picked.append(idx)
    if not picked:
        raise RuntimeError(f"找不到 phase={phase} task={task} 的样本")
    return picked
...
def main():
    ...
    parser.add_argument("--sample_index", type=int, default=None, help="指定单条样本下标，覆盖 --n")
    parser.add_argument("--phase", type=str, default=None, choices=PHASES, help="只展示指定阶段")
    parser.add_argument("--task", type=str, default=None,
                        choices=["play_rank", "response_choice"], help="只展示指定任务")
    ...
    hidden = ck_args.get("hidden", 384)
    val = ckpt.get("val_metrics", {})
    print(f"[ckpt] hidden={hidden}   epoch={ckpt.get('epoch')}   saved_valid_acc={val.get('acc')}")

    model = PointerPolicy(
        hidden=hidden, conv_blocks=ck_args.get("conv_blocks", 3),
        hist_layers=ck_args.get("hist_layers", 4), hist_heads=ck_args.get("hist_heads", 8),
    ).to(device).eval()
    ...
    if args.sample_index is not None:
        ...
        idxs = pick_indices(ds, path, args.n, args.phase, args.task, args.seed)

    n_correct = 0
    for k, idx in enumerate(idxs):
        raw = load_raw(path, ds.offsets[idx])
        batch = collate_fn([featurize(raw)])
        batch = {kk: vv.to(device) if torch.is_tensor(vv) else vv for kk, vv in batch.items()}
        with torch.no_grad():
            probs = F.softmax(model(batch), dim=-1)[0].cpu()
        pred_idx = int(probs.argmax().item())

        print(f"\n########## {k+1}/{len(idxs)}   sample idx={idx} ##########")
        print_sample(raw, probs, pred_idx)
        label = raw.get("label_index", raw.get("human_selected_index", 0))
        if pred_idx == label:
            n_correct += 1
